...
# refer to line 44 above if you need help but don't just copy/paste--type it in yourself!
...
# refer to lines 71, 72 and 78 above if you need help but do not copy/paste!
...
virginica6 <- filter(iris, Species == "virginica", Sepal.Length > 6)

tail(virginica6)  # shows the last 6 rows of the dataframe
```

```{r}
# generate a histogram showing the sample distribution of Sepal Width in virginica
...
```{r}
# use dplyr::select(data, column1, column2, ...) to select specified columns
Sepal <- dplyr::select(iris, Sepal.Length, Sepal.Width)
# select all columns from Sepal.Length to Petal.Length using a : to separate the beginning and end of the range and store the result in a dataframe called 'iris_subset'
...
SepalLengthSummary <- iris %>% 
  ...
  summarize(
    ...
    SepalLength_se = sd(Sepal.Length)/sqrt(length(Sepal.Length))  # See note below
    )
...
```{r}
# Use a shapiro test to test for normality in the set of virginica sepal widths
...
**Test 1**: Does the roundness of setosa sepals significantly differ from the roundness of virginica sepals (as defined by the RoundSepal category)?
...
**Test 2**: Is sepal width significantly different between virginica and setosa? 
...
input_data <- data.frame(Sepal.Width = c(3,4,5)) # this just creates some sample input data
...
```{r}
#Use the lm() function to generate a linear model and store it as 'Sepal_petal'

#plot the daata and the linear model using the plot() and abline() functions

```

```{r}
#Use cor.test() to determine the correlation between sepal length and petal length
...
**Test 4**:  Is there a significant difference in mean petal length among any of the species?
...
# The aov function creates the linear model automatically ()
...
```{r}
# An alternative to TukeyHSD is the pairwise.t.test
attach(iris)
pairwise.t.test(Petal.Length, Species, p.adjust.method = "bonf")
detach()
```

**Test 5**:  Is there a significant difference in mean petal length between virginica and either of the other two species?

```{r}
# Perform Dunnett's test to compare the petal length of the other two species to the control (virginica)

# First, make sure R sees the data in the Species column as a factor and set the level order of this factor
iris$Species <- as.factor(iris$Species) 
iris$Species <- factor(iris$Species, levels = c("virginica", "setosa", "versicolor")) 

# Then, create a linear model that predicts Petal Length as a function of Species
model <- lm(Petal.Length ~ Species, data = iris)

# Finally, use the General Linear Hypothesis Test (glht) to perform a Dunnett's Test, which will use the first level as a control
summary(glht(model, linfct = mcp(Species = "Dunnett")))
```

**Test 6**:  Is there a significant difference in the number of petals (using randomly generated data) between setosa and virginica?

```{r}
#code to generate random count data for the number of petals in setosa and virginica

set.seed(123)

#use different lambda values for each species so there will be a significant difference in count data between species
lambda_by_species <- c(
  setosa = 4,
  verginica = 6
)

VirgSeto$PetalCount_poisson <- rpois(
  n = nrow(VirgSeto),
  lambda = lambda_by_species[VirgSeto$Species]
)
...
```{r}
# Fit a Poisson regression model that allows each species to have its own expected count
VirgSeto_glm <- glm(PetalCount_poisson ~ Species, family = poisson(link = "log"), data = VirgSeto)

# Create a null model that assumes a shared mean for both species
VirgSeto_null <- update(VirgSeto_glm, . ~ 1)

# Test whether assuming species-specific means significantly improves the model.
lrt <- anova(VirgSeto_null, VirgSeto_glm, test = "Chisq")
