...
A p-value *greater* 0.05 means that the test does not reject the null hypothesis (i.e. that the data do not deviate significantly from a normal distribution)
...
```{r}
# filter using two criteria, species virginica and Sepal.Length greater than 6, separating each criterion with a comma
...
Low p-value indicates that a linear model of these data is appropriate; modest r-squared value indicates that the correlation is mediocre. What does this look like on a graph?
...
new_data <- data_frame(Sepal.Width = c(3,4,5))
predict(Sepal_lm, newdata = new_data)
```
...
#  exactly one parameter in pwr.t2n.test(n1 = NULL, n2 = NULL, d = NULL, sig.level = 0.05, power = NULL) must be NULL.  The function will  solve for the missing parameter
